...
//					@AuthenticationPrincipal 
		...
		try {
			// (1) TodoVo로 변환한다.
			TodoVo entity = TodoDTO.toEntity(dto);
			...
			List<TodoVo> entities = service.create(entity);
...
//					@AuthenticationPrincipal 
		...
		List<TodoVo> entities = service.retrieve(userId);
	...
	@GetMapping("/list")
	public ModelAndView webTodoList(
//					@AuthenticationPrincipal 
					String userId) {
		ModelAndView mv = new ModelAndView("todoList");
		System.out.println("webUserID : " + userId);
		// (1) 서비스 메서드의 retrieve메서드를 사용해 Todo리스트를 가져온다
		List<TodoVo> entities = service.retrieve(userId);
...
//			@AuthenticationPrincipal 
		...
		List<TodoVo> entities = service.update(entity);
...
//					@AuthenticationPrincipal 
		...
		try {
			...
			List<TodoVo> entities = service.delete(entity);
